                ...
                <tr key={art.id} className="transition-colors hover:bg-slate-50/50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <img src={art.imageUrl} alt={art.title} className="h-12 w-12 flex-shrink-0 rounded-lg object-cover ring-1 ring-slate-200" />
        ...
        <div className="block md:hidden divide-y divide-slate-100">
          {artworks.map((art) => (
            <div key={art.id} className="p-4 space-y-3">
              <div className="flex items-center gap-3">
                <img src={art.imageUrl} alt={art.title} className="h-16 w-16 flex-shrink-0 rounded-lg object-cover ring-1 ring-slate-200" />
                ...
                <button 
                  ...
                  className={`rounded-full px-2 py-1 text-[10px] font-semibold ${
                    art.status === 'sold' ? 'bg-rose-50 text-rose-600' : 'bg-violet-50 text-violet-700'
                  }`}
                >
                  {art.status === 'sold' ? 'Sold' : 'Available'}
                </button>
...
function ArtworkFormModal({ artwork, onSave, onClose }: { artwork: Artwork | null; onSave: (data: Omit<Artwork, 'id'>) => void; onClose: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !medium.trim() || !dimensions.trim() || !imageUrl.trim()) { setError('Please fill in all required fields.'); return; }
    const price = parseInt(pricePKR, 10);
    if (isNaN(price) || price < 0) { setError('Please enter a valid price.'); return; }
    onSave({ title: title.trim(), category, medium: medium.trim(), dimensions: dimensions.trim(), pricePKR: price, status, imageUrl: imageUrl.trim(), description: description.trim() || undefined });
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 px-4 sm:px-6 py-4 sm:py-5">
          {imageUrl && (
            <div className="overflow-hidden rounded-xl ring-1 ring-slate-200">
              <img src={imageUrl} alt="Preview" className="h-36 sm:h-44 w-full object-cover" />
            </div>
          )}
