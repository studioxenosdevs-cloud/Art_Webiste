...
function getStatusConfig(status: string) {
    return STATUS_CONFIG[status as InquiryStatus] ?? STATUS_CONFIG.new;
}
                    ...
                    {filtered.map((order, i) => (
                        <div key={order.id} className="reveal glass-card glass-card-hover overflow-hidden rounded-xl" style={{ animationDelay: `${Math.min(i * 0.05, 0.3)}s` }}>
                            <div className="flex cursor-pointer items-center gap-4 px-4 py-3.5 transition-colors hover:bg-slate-50/50" onClick={() => setExpanded(expanded === order.id ? null : order.id)}>
                                ...
                                <select value={ALL_STATUSES.includes(order.status) ? order.status : 'new'} onClick={(e) => e.stopPropagation()} onChange={(e) => updateOrderStatus(order.id, e.target.value as InquiryStatus)} className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 outline-none transition-all ${getStatusConfig(order.status).classes} cursor-pointer`}>
