...
import { Palette, LayoutDashboard, Image as ImageIcon, Inbox, Settings, LogOut, Chrome as Home, Menu, X, Star, Sparkles } from 'lucide-react';
import { useAuth } from '@/hooks/useAuth';

type TabKey = 'overview' | 'artworks' | 'timeline' | 'inquiries' | 'settings' | 'reviews';
...
const NAV_ITEMS: { key: TabKey; label: string; icon: React.ReactNode; to: string }[] = [
    ...
    { key: 'timeline', label: 'Timeline & Journey', icon: <Sparkles className="h-5 w-5" />, to: '/admin/timeline' },
    { key: 'inquiries', label: 'Inquiries', icon: <Inbox className="h-5 w-5" />, to: '/admin/inquiries' },
    { key: 'reviews', label: 'Reviews', icon: <Star className="h-5 w-5" />, to: '/admin/reviews' },
                ...
                <nav className="flex-1 space-y-1 px-3 py-4">
                    {NAV_ITEMS.map((item) => (
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${isActive ? 'bg-violet-50 text-violet-700 ring-1 ring-violet-600/20' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'
                                }`
                            }
                        >
                            {item.icon}
