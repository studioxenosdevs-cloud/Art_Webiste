    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in bg-slate-900/80 backdrop-blur-md" onClick={onClose}>
      ...
      <div className="relative flex max-h-[92vh] w-full max-w-5xl flex-col gap-6 px-4 py-4 animate-zoom-in lg:flex-row lg:items-center" onClick={(e) => e.stopPropagation()}>
        <div className="relative flex flex-1 items-center justify-center overflow-hidden rounded-2xl bg-white/5">
          <img src={art.imageUrl} alt={art.title} className={`lightbox-img max-h-[70vh] w-auto max-w-full rounded-xl object-contain shadow-2xl ${zoomed ? 'zoomed' : ''}`} onClick={() => setZoomed((z) => !z)} />
        ...
        <div className="flex w-full flex-col gap-4 lg:w-80 lg:flex-shrink-0">
          <div>
            <span className={`mb-2 inline-block rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ring-1 ${
              art.status === 'sold' ? 'bg-rose-50 text-rose-600 ring-rose-500/30' : 'bg-violet-50 text-violet-700 ring-violet-600/30'
            }`}>
          ...
          <button
            ...
            className={`flex w-full items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold transition-all ${
              art.status === 'sold' ? 'cursor-not-allowed bg-white/5 text-slate-500' : 'bg-gradient-to-r from-violet-600 to-violet-700 text-white hover:shadow-[0_0_30px_-5px_rgba(124,58,237,0.5)] hover:scale-[1.02]'
            }`}
          >
