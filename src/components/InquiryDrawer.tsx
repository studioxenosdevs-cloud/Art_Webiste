...
interface InquiryDrawerProps {
    ...
    onClose: () => void;
    onSubmit: (order: Omit<Order, 'id' | 'createdAt' | 'status'>) => Promise<void>;
}
...
export default function InquiryDrawer({ artwork, onClose, onSubmit }: InquiryDrawerProps) {
    ...
    const validate = (): boolean => {
        ...
        if (!sanitizeText(form.customerName, 120)) e.customerName = 'Please enter your name';
        if (!sanitizeText(form.phone, 40)) e.phone = 'Please enter your WhatsApp/phone number';
        if (!sanitizeText(form.shippingAddress, 1_000)) e.shippingAddress = 'Please enter your delivery address';
        if (form.email.trim() && !isValidEmail(form.email.trim())) e.email = 'Please enter a valid email address';
        setErrors(e);
    ...
    const handleSubmit = async (ev: React.FormEvent) => {
        ...
        try {
            ...
            await onSubmit({
                ...
                customerName: sanitizeText(form.customerName, 120),
                email: form.email.trim() || undefined,
                phone: sanitizeText(form.phone, 40),
                shippingAddress: sanitizeText(form.shippingAddress, 1_000),
        ...
        <>
            ...
            <div className="drawer-panel fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl">
                ...
                <div className="flex items-center gap-3 border-b border-slate-200 bg-slate-50/50 px-5 py-3">
                    {!isCommission && <img src={artwork.imageUrl} alt={artwork.title} className="h-14 w-14 flex-shrink-0 rounded-lg object-cover ring-1 ring-slate-200" loading="lazy" decoding="async" />}
                    {isCommission && (
