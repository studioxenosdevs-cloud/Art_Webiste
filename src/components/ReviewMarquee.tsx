...
export default function ReviewMarquee() {
    ...
    useEffect(() => {
        ...
        async function loadApprovedReviews() {
            try {
                ...
                if (error) {
                    // Fallback query if table schema uses camelCase or lacks created_at index
                    const fallback = await supabase
                        .from('reviews')
                        .select('*');
                    if (fallback.data && isMounted) {
                        const all = (fallback.data as ReviewRow[]).map(mapReviewFromDB);
                        setApprovedReviews(all.filter((r) => r.isApproved));
                    }
                } else if (data && isMounted) {
                    setApprovedReviews((data as ReviewRow[]).map(mapReviewFromDB));
                }
            } catch (err) {
        ...
        const channel = supabase
            .channel('public:reviews-marquee')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setSubmitError('');
        const safeAuthor = sanitizeText(author, 100) || 'Anonymous';
        const safeComment = sanitizeText(comment, 1_500);
        if (!safeComment) {
            setSubmitError('Please write a review before submitting.');
            return;
        }
        if (!Number.isInteger(rating) || rating < 1 || rating > 5) {
            setSubmitError('Rating must be a whole number from 1 to 5.');
            return;
        }
        setSubmitting(true);
        try {
            ...
            // Do not request the inserted row here: public RLS intentionally
            // hides unapproved reviews, and PostgREST would reject RETURNING.
            ...
            // Cache for non-logged-in public visitor
            const { data: sessionData } = await supabase.auth.getSession();
            if (!sessionData?.session?.user) {
                try {
                    const mapped: Review = {
                        id: `local-${Date.now()}`,
                        author: sanitizedReview.author_name,
                        message: sanitizedReview.comment,
                    ...
                    const updated = [mapped, ...list.filter((r) => r.id !== mapped.id)];
                    localStorage.setItem(REVIEWS_CACHE_KEY, JSON.stringify(updated));
                } catch (cacheErr) {
                    console.warn('Failed to cache review locally:', cacheErr);
                }
            }
                        ...
                        {submitted ? (
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    ...
                                    <input className="form-input" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Anonymous" maxLength={100} />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <textarea className="form-input resize-none" rows={4} value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Share your experience…" maxLength={1500} required />
                                </div>
