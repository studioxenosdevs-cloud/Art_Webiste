          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {FILTERS.map((f) => (
              <button
                ...
                className={`pill-filter flex items-center gap-1.5 rounded-full border px-4 py-2 text-sm font-medium ${
                  activeFilter === f.key
                    ? 'active'
              ...
              >
                ...
                <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${
                  activeFilter === f.key ? 'bg-violet-100 text-violet-700' : 'bg-slate-100 text-slate-400'
                }`}>
                  {counts[f.key] ?? 0}
                </span>
              </button>
  ...
  return (
    <article className="card-sleek glass-card glass-card-hover group relative flex flex-col overflow-hidden rounded-2xl" style={{ animation: `fadeInUp 0.6s ease ${Math.min(index * 0.08, 0.4)}s both` }}>
      <div className="relative aspect-[3/4] overflow-hidden">
        <img src={art.imageUrl} alt={art.title} className="h-full w-full object-cover image-zoom" loading="lazy" />
        ...
        <div className="mt-auto flex items-center justify-between pt-4">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold transition-all ${
              sold ? 'cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-violet-50 text-violet-700 ring-1 ring-violet-600/30 hover:bg-violet-100 hover:scale-105'
            }`}
          >
